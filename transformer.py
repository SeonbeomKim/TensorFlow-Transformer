...
class Transformer:
	def __init__(self, sess, voca_size, embedding_size, is_embedding_scale, PE_sequence_length,
		...
		with tf.name_scope("placeholder"):
			...
			self.feed_encoder_embedding = tf.placeholder(tf.float32, [None, None, self.embedding_size]) # for inference
			self.beam_width = tf.placeholder(tf.int32)
		...
		with tf.name_scope("masks"):
			...
			self.expand_dim_encoder_input_mask = tf.expand_dims(
		...
		with tf.name_scope('encoder'):
			encoder_input_embedding = self.embedding_and_PE(self.sentence, self.sentence_length)
			self.encoder_embedding = self.encoder(encoder_input_embedding, self.expand_dim_encoder_input_mask)


		with tf.name_scope('decoder'):
			decoder_input_embedding = self.embedding_and_PE(self.target, self.target_length)
			...
			self.decoder_pred_except_eos += (eos_mask_of_decoder_pred - 1) # the value of the masked position is -1
	...
	def beam_search_graph(self, infer_embedding, time_step, beam_width):
		# infer_embeding: [N*beam_width, target_length, self.voca_size]
	...
	def encoder(self, encoder_input_embedding, encoder_input_mask):
		# encoder_input_embedding masking
	...
	def multi_head_attention_add_norm(self, query, key_value, mask=None, activation=None, name=None):
		...
		with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
			...
			# encoder multi-head attention, encoder-decoder attention
			else:
				query_not_zero = tf.cast(tf.not_equal(query, 0.), dtype=tf.float32)
				key_value_not_zero = tf.cast(tf.not_equal(key_value, 0.), dtype=tf.float32)
				query_key_value_mask = tf.cast(tf.matmul(query_not_zero, tf.transpose(key_value_not_zero, (0, 2, 1))) > 0, dtype=tf.float32)
				tile_query_key_value_mask = tf.tile(query_key_value_mask, [8, 1, 1]) # batch 8배. 
				
				score = score * tile_query_key_value_mask # zero mask
				score = score + ((tile_query_key_value_mask-1) * 1e+10) # -inf mask				
