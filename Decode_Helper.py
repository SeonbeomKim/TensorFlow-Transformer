...
class greedy_decoder:		
	def decode(self, decoder_fn, encoder_embedding, target_length, output_embedding_table, PE, go_idx):
		...
		for index in range(target_length): 
			...
			current_output = tf.expand_dims(current_output, axis=1) # [N, 1, self.voca_size]
			decoder_output.append(current_output) 
			argmax_current_output = tf.argmax(current_output, axis=-1) # [N, 1]
...
class beam_decoder:
	...
	def decode(self, decoder_fn, encoder_embedding, target_length, output_embedding_table, PE, go_idx):
		...
		decoder_input = tf.contrib.seq2seq.tile_batch(
					decoder_input, 
					self.beam_size
				) # [N*self.beam_size, self.target_length, self.embedding_size]
		...
		for index in range(target_length): 
			...
			current_output = decoder_fn(
						decoder_input, #decoder_input은 index마다 업데이트됨. 
						encoder_embedding, 
					) # [N*self.beam_size, self.target_length, self.voca_size]
			...
			if index < target_length-1:	
				pad_top_k_indices = tf.pad( 
							...
							mode='CONSTANT', 
							constant_values=-1
						) # [N*beam_size, self.target_length]
				
				embedding_pad_top_k_indices = tf.nn.embedding_lookup(
							output_embedding_table, 
							pad_top_k_indices
						) # [N*beam_size, self.target_length, self.embedding_size]		
		...
		# e^k == a*b*c
		# a*b*c == tf.exp(previous_top_k)

		return previous_top_k_indices, tf.exp(previous_top_k_prob)
